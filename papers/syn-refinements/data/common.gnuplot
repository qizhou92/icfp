...
mpNextOptimusNoTitle = '\
    eval mpNext; \
    set xrange [0:80]; \
    set xtics 0,10,80'

mpNextGoldbugNoTitle = '\
    eval mpNext; \
    set xrange [0:32]; \
    set xtics 0,8,32'

mpNextXeonPhiNoTitle = '\
    eval mpNext; \
    set xrange [0:57]; \
    set xtics 0,8,60'

mpNextOptimus = '\
    eval mpNextOptimusNoTitle; \
    set title "80-core Xeon"'

mpNextGoldbug = '\
    eval mpNextGoldbugNoTitle; \
    set title "32-core Xeon"'

mpNextXeonPhi = '\
    eval mpNextXeonPhiNoTitle; \
    set title "57-core Xeon Phi"'

mpNextSharing = '\
    eval mpNext; \
    if (mp_nplot>0) {unset key}; \
    if (mp_nplot%3==0) {@xaxis_optimus}; \
    if (mp_nplot%3==1) {@xaxis_goldbug}; \
    if (mp_nplot%3==2) {@xaxis_bumblebee_mic}; \
    if (mp_nplot==0) {set title "80-core Xeon"}; \
    if (mp_nplot==1) {set title "32-core Xeon"}; \
    if (mp_nplot==2) {set title "57-core Xeon Phi"}; \
    if (mp_nplot>2) {unset title}'

mpNextMemscan = '\
    eval mpNext; \
    if (mp_nplot>2) {unset key}; \
    if (mp_nplot%3==0) {@xaxis_host}; \
    if (mp_nplot%3==1) {@xaxis_host}; \
    if (mp_nplot%3==2) {@xaxis_mic}; \
    if (mp_nplot==0) {set title "80-core Xeon"}; \
    if (mp_nplot==1) {set title "32-core Xeon"}; \
    if (mp_nplot==2) {set title "57-core Xeon Phi"}; \
    if (mp_nplot>2) {unset title}'
