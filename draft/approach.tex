...
%
\BH{fix defn to include derivations of subexpressions}
%
  ...
  \begin{algorithm}[H]
    ...
    { \myproc{$\chkindaux(n, I)$ %
      ...
      { % n is inductive under D,
        \lIf{$I(\dersexps{D}(n)) \entails \dersrelinvs{D}(n)$ %
          \label{line:chkind-is-entailed} } {
          % case: node is inductive:
          \Return{$\true$} } %
        % 
        \Else{ %
          $I' = \upd{I}{ \dersexps{D}(n) }{ %
            I(\dersexps{D}(n)) \land \invsof{I}(n) }$ %
          \label{line:str-invs} \;
          \lIf{$\biglor \setformer{ %
              \chkindaux(n_0, I') = \chkindaux(n_1, I') = \isind }{ %
              n_0, n_1 \in \nodes{D}, (n_0, n_1, n') \in T }$}{ %
            \Return{$\true$} \label{line:decomp-isind} } %
          \lElse{ %
            \Return{$\chooseres( %
              \unwind{ P_0 }(n, D, I'), %
              \unwind{ P_1 }(n, D, I'))$ %
