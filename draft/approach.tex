...
\begin{figure}
  ...
  \begin{algorithm}[H]
    ...
    \SetKwProg{myproc}{Procedure}{}{}
    % Inputs: a program
    \Input{$T \in \tilings{P_0}{P_1}$, $I \in \dersinvs{P_0}{P_1}$
      such that $(T, I) \in \dersinvs{P_0}{P_1}$.
      %
      \BH{fix spaces}
      }
    % Output: decision for safety
    \Output{$\isind$ to denote that some restriction of $(T, I)$ are
      inductive or derivations not defined in $I$.}
    % verify: main procedure
    \myproc{$\chkinductive(T, I)$ \label{line:chkind-begin}} %
    { \myproc{$\chkindaux(O, I')$ \label{line:vders-aux-begin} } %
      { % check set of obligations:
        \Switch{$O$} %
        { \lCase{$\emptyset$} {
            % case: no obligations remain:
            \Return{$\isind$}
          } %
          \Case{$\elts{ (\mathcal{D}_0, \mathcal{D}_1) } \union O'$}{
            % check if obligation is undefined in I:
            \lIf{$(\mathcal{D}_0, \mathcal{D}_1) \notin \domain(I)$}{ %
              % obligation isn't defined. Return derivations:
              \Return{ $(\chooseder(\mathcal{D}_0), %
                \chooseder(\mathcal{D}_1)$ } 
            } %
            \Else{ %
            } %
          } %
      ...
      % base call:
      \Return{$\chkindaux(\elts{ ([ P_0 ], [ P_1 ]) }, %
        \alltrue)$} \label{line:chkind-base}
    } %
    % 
  ...
  \end{algorithm}
\end{figure}
\BH{define algorithm that determines inductiveness of given relational
  invariants}
