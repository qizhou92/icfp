...
%
For $M : I \partto X$, the \emph{mutual} least fixed point of $\tau$
over $M$ is
$\mu_M \tau = \mu_{i_0} M(i_0). \mu_{i_1} M(i_1) \ldots \tau$ for all
indices $i_k$ in the domain of $M$.

\paragraph{Replacing components of relational refinement types}
%
...
$\tau'$.
% define closure of indexed lfp vars under a set of 
For space of indices $I$, $\mathcal{X} \subseteq \lfpvars$, and
$M \in \pset(\idxpos{I})$, $\mathcal{X}$ is \emph{closed} under
$\mathcal{M}$ if for all $i, j \in I$ and all $p, q \in \typos$...
% range over sets of indexed type positions:
$\mathcal{X}$ is closed under
$\mathcal{M} \subseteq \pset(\idxpos{I})$ if $\mathcal{X}$ is closed
under $M$ for all $M \in \mathcal{M}$.

% define replacement:
For $\Phi, \Phi' \subseteq \refpreds{ \idxpos{I} }$ and %
$\mathcal{M}' \subseteq \pset(\idxpos{I})$, the replacement of
$(\Phi', \mathcal{M}')$ in $(\Phi, \mathcal{M})$ at $\mathcal{X}$,
denoted
$\replace{(\Phi, \mathcal{M})}{X}{(\Phi',
  \mathcal{M}')}_{\mathcal{X}}$, is $(\Phi'', \mathcal{M}'')$, where
%
\begin{align*}
  % refinement predicates are base refinement predicates and
  \Phi'' = & \Phi \union %
             % concatenation of M and phi
             \setformer{ M \concat \varphi' }{ 
             % for each mutual induction set M in base type and
             M \in \mathcal{M}_{\mathcal{X}},
             % each refinement predicate in substituted type,
             \varphi' \in \Phi'_{\mathcal{X}} } \\
  % mutual induction sets are base mutual induction sets and
  \mathcal{M}'' = & \mathcal{M} \union %
                    % concatenation of M and M'
...
%
\BH{complete: define subtyping relation}
