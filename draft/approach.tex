...
\begin{defn}
  ...
  \begin{itemize}
  \item % empty:
    $\tau$ is the type in $\tau$ at $\epsilon$.
  \item % function:
...
\begin{defn}
  ...
  are \emph{relatable} if one of the following conditions holds:
  \begin{itemize}
  \item % identity:
    Each $p \in \typos$ is relatable with itself;
  \item % distinct factors:
    ...
    $q_r \concat [ \tyrightprod ] \concat p$ are relatable.
  \end{itemize}
\end{defn}
%
...
components of indexed type.
%
\begin{defn}
  \label{defn:ref-refinements}
  For indices $I$ and $t : I \to T$, $\varphi \in \idxpos{I}$ is a
  \emph{relational refinement predicate} of $t$ if the following
  conditions hold:
  \begin{itemize}
  ...
  \item % vocab are all relatable,
    ...
    $(i, p), (i, q) \in \vocab(\varphi)$, it holds that $p \sim q$.
  \end{itemize}
\end{defn}

% introduce inductive defns:
Each relational refinement type also contains directives for how
inductive definitions at distinct indices should be mutually unfolded
in order to express a property.
%
...
\begin{defn}
  ...
  \item % no distinct positions that extend an inductive type:
    ...
    $(i, q_0), (i, q_1) \in P$, it holds that $q_0 = q_1$.
  \end{itemize}
\end{defn}
...
\begin{defn}
  ...
  For indices $I$, a \emph{relational type} over $I$ is a pair
  $(\Phi, M)$ where for some $t : I \to T$,
  % 
  \begin{itemize}
  \item % refinement predicates,
    $\Phi \subseteq \relpreds{ \idxpos{I} }$ are refinement predicates
    of $t$.
  \item % mutual induction defn,
    $M \subseteq \pset(P)$ are a complete mutual induction set of $t$.
  \end{itemize}
\end{defn}
%
...
\paragraph{Operations and relations over relational refinement types}
...
\BH{lower this to when we define operations or the subtyping relation}
...
\BH{update to define all notation as needed}
