...
\textbf{(3)} $P : X \to \typos$;
%
\textbf{(4)} $E \subseteq X \times X$ an equivalence relation over
$X$.
%
...
Let $d_P : \mathcal{X} \to \mathcal{X} \times \typos$ be such that for
each $X \in \mathcal{X}$, $d_P(X) = (X, P(X))$.
%
For each $P : \mathcal{X} \to \typos$, let
$d_{P} : \mathcal{X} \to \mathcal{X} \times \typos$ be such that for
each $X \in \mathcal{X}$, $d_{P}(X) = (X, P(X))$.
% define selector:
For $N : \mathcal{X} \to I$, $i \in I$, and $p \in \typos$, let
$s_{i, p}^N : \mathcal{X} \times \typos \to \typos$ be such that for
each $X \in \lfpvars$ and $q \in \typos$, if $N(X) \not= i$, then
$s_{i, p}^N(X, q) = q$, and otherwise
$s_{i, p}^N(X, q) = p \concat q$.
%
The \emph{prefix} of $P$ with $p$ at $i$ under $N$ is
$p \fullprepend{i}{N} P = d_{P} \compose s_{i, p}^N$.
%
For $\Phi : \idxrefpreds{ I }$, $N : \lfpvars \to I$,
$P : \lfpvars \to \typos$, and $E \subseteq \in \equiv_{\mathcal{X}}$
the prepending of $(\Phi, N, P, E)$ with $p$ at $i$ is
...
$m : I \partto \typos$, the \emph{prefixing} of
$\Phi \in \idxrefpreds{ I }$ with $m$ is
$m(i_n) \prepend{i_n} \ldots m(i_0) \prepend{i_0} \Phi$, where $i_k$
range over $\domain(m)$.
%
The prefixing of each $P : \mathcal{X} \to \typos$ with $m$ is
$m(i_n) \prepend{i_n} \ldots m(i_0) \prepend{i_0} P$, where $i_k$
range over the $\domain(m)$.
...
$\replace{ (\Phi, N, P, \mathcal{M}) }{ \mathcal{Y} }{ (\Phi', N, P',
  \mathcal{M}) } = (\Phi \union m \concat \Phi', N, P \union m \concat
P', \mathcal{M})$, where
