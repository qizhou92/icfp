...
\begin{figure}
  ...
  \begin{gather*}
    % refine atomics:
    \inference[ ATOMICS ]{ }{ } \\
    % refine when relational type is function
    \inference[ FNS-REL ]{
      % get refinement for parameter,
      \tau_0 \refines{ i }{ t }{ \tau } \tau_1 &
      % get refinement for result,
      \tau_0' \refines{ i }{ t }{ \tau } \tau_1' }{
      \tau_0 \to_x^j \tau_0' \refines{ i }{ t }{ \tau }
      \tau_1 \to_x^j \tau_1' }
    % subtype atomic with sum
    \inference[ SUM-REL ]{
      % get refinement for lhs,
      \tau_l \refines{i}{t}{\tau} \tau_l' &
      % get refinement for rhs,
      \tau_r \refines{i}{t}{\tau} \tau_r' }{
      % complete type is sum
      \tau_l +^j \tau_r \refines{i}{t}{\tau} \tau_l' +^j \tau_r' } \\
    % subtype atomic with product
    \inference[ PROD-REL ]{
      % get refinement for nested type,
      \tau' \refines{i}{t}{\tau} \tau'' }{
      % complete type is product
      \times_{j, k}^l \tau' \refines{i}{t}{\tau} %
      \times_{j, k}^{l} \tau'' } 
    % refine relational LFP
    \inference[ LFP-REL ]{ }{ } \\
    % refine individual function type:
    \inference[ FNS-IND ]{ }{
      % compose individual function type
      A \refines{ i }{ t }{ \tau_i \to \tau_i' } A' }
    % refine individual sum type:
    \inference[ SUM-IND ]{ }{
      % complete type is sum
      A \refines{i}{t}{ \tau_i^l + \tau_i^r } A' } \\
    % refine individual product type:
    \inference[ PROD-IND ]{ }{
      % complete type is product
      A \refines{i}{t}{\tau_i^l \times \tau_i^r} %
      A' } 
    % refine individual lfp type:
    \inference[ LFP-IND ]{ }{ } \\
  \end{gather*}
  \caption{Inductive inference rules that define the refinement relation over relational refinement types.
    %
    \BH{define metavariables used in rules} }
  \label{fig:refine}
\end{figure}

% define the relational refinement relation:
...
% walk through the inductive inference rules:
The refinement relation is defined by the inductive inference rules
...
\begin{figure}
  ...
  \begin{gather*}
    ...
    \inference[ ATOMICS ]{ }{ \Phi \subtype \Phi' } \\
    % subtype function with atomic
    \inference[ FNS-L]{ \tau \subtype A & A \subtype \tau' }{
      A \subtype \tau \rightarrow_x^i \tau'} 
    % subtype atomic with sum
    \inference[ SUM-L ]{ A \subtype \tau & A \subtype \tau' }{
      A \subtype \tau +^i \tau' } 
    % subtype atomic with product
    \inference[ PROD-L ]{ A \subtype \tau }{
      A \subtype \times^k_{i, j} \tau } \\
    % subtype function with atomic
    \inference[ FNS-R]{ A \subtype \tau & \tau' \subtype A }{
      \tau \rightarrow_x^i \tau' \subtype A} 
    % subtype sum with atomic
    \inference[ SUM-R ]{ \tau \subtype A & \tau' \subtype A }{
      \tau +^i \tau' \subtype A} 
    % subtype product with atomic
    \inference[ PROD-R ]{ \tau \subtype A }{
