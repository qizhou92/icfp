...
programs.
%
\autoref{sec:rel-type-space} defines the representation types, %
\autoref{sec:rel-type-ops} defines the space of types as the closure
under type operations, and %
...
predicates and an association of least-fixed point types.
%
\begin{defn}
  ...
  % 
  \textbf{(4)} $\mathcal{M} \subseteq X \times X$ is an equivalence
...
relational type representations closed under the following set of
operations, which correspond to the space of constructors that define
a conventional space of types (e.g., the space of structural types
defined in \autoref{sec:stlc}).

\paragraph{Atomic relational refinement types}
%
% introduce map
For indices $I$, let $f : \refpreds{I} \to \refpreds{\idxpos{I} }$ be
such that for each $\varphi \in \refpreds{I}$,
