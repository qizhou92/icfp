...
\BH{define positioned expressions: only need to record pairs and base
  operations. Examples: the empty map, mapping from positions to
  variables}
...
\BH{define refinement}

\BH{define equivalence extension}
...
\begin{figure}
  ...
  \begin{gather*}
    ...
    % vars:
    \inference[ VARS ]{ }{ %
      \Gamma, x : \tau \judges P \mapsto X \concat [ x ] \concat X' : %
      \tau \joinctr (P \equiv X \concat [ x ] \concat X') } \\
    % constant:
    \inference[ CONST ]{ %
      % remainder has type tau,
      \Gamma \judges E : \tau }{ %
      \Gamma \judges \upd{ E }{ p }{ c } : \tau \joinctr_p c } \qquad
    % base operation:
    \inference[ OP ]{ %
      % remainder has type tau,
      \Gamma \judges %
    ...
    \inference[ LET ]{
      ...
      \Gamma \judges \upd{ E }{p}{ e_0 } : \times_{x, y}^p \tau &
      % get type of result under context that binds to x and y,
      \Gamma, x, y : \tau \judges e_1 : \tau' }{
...
% empty:
Rule~EMPTY establishes that the empty positional expression map has
the empty relational type.

% variables:
Rule~VARS establishes that under a context that binds variable $x$ to
relational type $\tau$, the positional expressions that map positions
$P$ to variables $X \concat [ x ] \concat X'$ have as a type $\tau$
extended to set $P$ equivalent to $X \concat [ x ] \concat X'$.
% const:
Rule~CONST establishes that if positioned expressions have type
...
$e_0 \oplus e_1$ has type $\tau$ refined to set $p$ to
$\leftopchild \cons p \oplus \rightopchild \cons p$.
...
operator.
%
Rule~ABS establishes that if, under context $\Gamma$ extended to bind
variable $x$ to relational type $\tau$, $E$ extended to bind $p$ to
expression $e$ has type $\tau'$, then under $\Gamma$, $E$ extended to
bind $p$ to $\lambda x. e$ is a relational function from $\tau$ to
$\tau'$ at $p$.
% app:
...
relational product of $\tau$ from variables $x$ and $y$ to $p$;
%
\textbf{(2)} under $\Gamma$ extended to bind $x$ and $y$ to $\tau$,
$e_1$ has type $\tau'$, then $E$ extended to bind $p$ to
$\letexp{x}{y}{e_0}{e_1}$ has type $\tau'$.
...
relational from of $\tau$ and $\tau'$ from variables $x$ and $y$ to
$p$;
% 
\textbf{(2)} under $\Gamma$ extended to bind $x$ to $\tau$, $E$
extended to bind $p$ to $e_1$ has type $\tau''$; and
%
\textbf{(3)} under $\Gamma$ extended to bind $y$ to $\tau'$, $E$
extended to bind $p$ to $e_2$ also has type $\tau''$, then $E$
extended to bind $p$ to $\matchexp{e_0}{x}{e_1}{y}{e_2}$ has type
$\tau''$.

% fix:
Rule~FIX establishes that if %
\textbf{(1)} under $\Gamma$ extended to bind $f$ to $\tau$, $E$
extended to bind $p$ to $e$ has type $\tau'$; and %
\textbf{(2)} $\tau'$ with $p$ renamed to $f$ is a subtype of $\tau'$,
then under $\Gamma$, $E$ extended to bind $p$ to $\fix{f}{e}$ has type
$\tau'$.
% unfold:
