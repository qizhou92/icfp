      ...
      \Switch{$\chkinductive(I)$ \label{line:chkind} }{ %
        ...
        \Case{$D_0 \in \dertrees{ \cc{P}_0 }, %
          ...
          \Switch{$\verifyders(D_0, D_1)$ \label{line:verify-ders} }{ %
            ...
            \Case{$I' \in \dersrels{P_0}{P_1}$: %
              \label{line:case-ders-invs} }{ %
...
\end{algorithm}
  \end{minipage}
  % algorithm for verifying a pair of derivations:
  \begin{minipage}{0.48\linewidth}
\begin{algorithm}[H]
  % Declare IO markers.
  \SetKwInOut{Input}{Input}
  ...
  \Output{Derivation relational invariants of $D_0$ and $D_1$ or the
    value $\nonequiv$.}
  % verify: main procedure
  \myproc{$\verifyders(D_0, D_1)$ \label{line:verify-ders-begin}} %
  { \myproc{$\verifydersaux(E)$ \label{line:ders-aux-begin} }{ %
      % construct CHC for pair of derivation and tilings:
      $\mathcal{S} \assign \derschc(D_0, D_1, E)$ %
      \label{line:defn-chc} \;
      % try to solve system:
      \Switch{$\solvechc(\mathcal{S})$ %
        \label{line:solve-chc} }{ %
        \Case{$\sigma \in \interps{ \mathcal{S} }$}{ %
          % case: system is solvable. Return solution:
          \Return{$(E, \sigma)$} \label{line:ret-pf} %
        } %
        \Case{$D \in \ders{ \mathcal{S} }$ %
          \label{line:case-chc-der}}{ %
          % check if D is valid counter-example:
          \Switch{$\isvalidcex(D)$ \label{line:chk-cex}}{
            \Case{$\isvalid$}{
              % subcase: the counterexample is valid:
              \Return{$\nonequiv$} } %
            \Case{$C \in \mathcal{S}$}{ %
              \Return{ $\verifydersaux(\remove{ E }{ \decomp(C) })$ }
            }
          } %
        } %
      } %
