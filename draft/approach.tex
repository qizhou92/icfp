...
%
Then for indices $I$, 
% each set of refinement predicates over trivial type positions,
each $\Phi \subseteq \refpreds{ I \times [ \lfpvars] }$,
% each mutual induction set,
$T : I \to [ \lfpvars ]$, and $\mathcal{M} \subseteq \pset(I)$, define
the \emph{atomic} relational type
$A_{\Phi, T, \mathcal{M}} = (\Phi, T(\mathcal{M}))$.
...
% define type position field prepended to formulas
For indices $I$, $i, j \in I$, $\varphi \in \refpreds{ \idxpos{I} }$,
and $f \in \typosfields$, $f$ prepended to $\varphi$ is
$f \prepend{i}{j} \varphi = \varphi[ \setformer{ (j, f \cons p) /
  (i, f)}{ p \in \typos } ]$.
% 
For $\Phi \subseteq \refpreds{ \idxpos{I} }$, $f$ prepended to $\Phi$
is
$f \prepend{i}{j} \Phi = \setformer{ f \prepend{i}{j} \varphi }{
  \varphi \in \Phi }$.
% define type position field prepended to mutual induction sets,
For $M \subseteq \idxpos{I}$, $f$ prepended to $M$ is
$f \prepend{i}{j} M = \setformer{ (i, f \cons p) }{ p \in \typos,
  (i, p) \in M}$.
%
For $\mathcal{M} \subseteq \pset(\idxpos{I})$, $f$ prepended to
$\mathcal{M}$ is
$f \prepend{i}{j} \mathcal{M} = \setformer{ f \prepend{i}{j} M }{ M
  \in \mathcal{M} }$.
...
$(\Phi, \mathcal{M}) \to^i_x (\Phi', \mathcal{M}') = (\typaramfield
\prepend{i}{i} \Phi \union \Phi'', \typaramfield \prepend{i}{i}
\mathcal{M} \union \mathcal{M}'')$, where
%
\begin{align*}
  % renamed result predicates,
  \Phi'' \equiv & \setformer{ %
                  \varphi[ (j, \typaramfield \cons p) / %
                  (x, \tyresfield \cons p) ] }{ %
                  \varphi \in \tyresfield \prepend{i}{i} \Phi' } \\
  % tricky mutual induction sets are 
  \mathcal{M}'' \equiv & \setformer{
                         \setformer{ (j, p) }{ %
                         j \not= x, p \in \typos, (j, p) \in M } \union
                         \setformer{ (i, \typaramfield \cons p) }{ %
                         p \in \typos, (x, \tyresfield \cons p) \in M } }{
                         M \in \tyresfield \prepend{i}{i} \mathcal{M}' }
\end{align*}
...
$k$ is
$(\Phi, \mathcal{M}) +_{i, j}^k (\Phi', \mathcal{M}') = (\tyleftsum
\prepend{i}{k} \Phi \union \tyrightsum \prepend{j}{k} \Phi',
\tyleftsum \prepend{i}{k} \mathcal{M} \union \tyrightsum
\prepend{j}{k} \mathcal{M}')$.
...
$\times_{i, j}^k (\Phi, \mathcal{M}) = (\tyleftprod \prepend{i}{k}
\tyrightprod \prepend{j}{k} \Phi, \tyleftprod \prepend{i}{k}
\tyrightprod \prepend{j}{k} \mathcal{M})$.
%
\BH{only let this be defined in cases in which resulting mutual
  induction sets work}
...
$(\Phi, \mathcal{M})$ at $i$ is
$\mu_i X. (\Phi, \mathcal{M}) = (\mu_X \prepend{i}{i} \Phi, \mu_X
\prepend{i}{i} \mathcal{M})$.
