...
$e_0, \ldots, e_n : \tau$ by finding types for non-recursive
expressions that under-approximate the behaviors of
$e_0, \ldots, e_n$.
%
Expression $e \in \expr$ is \emph{bounded} if it does not contain a
sub-expression of the form $\fix{f}{e'}$ for some $f \in \fixvars$ and
$e' \in \expr$.
    ...
    \myproc{$\inferaux(e_0', \ldots, e_n')$ %
      ...
      \Switch{$\inferbnd(e_0', \ldots, e_n', \tau)$ %
        ...
        \Case{$T \in \subexptypes{ e_0, \ldots, e_n }{ \tau }$ %
          ...
          \lElse{
            \Return{ $\inferaux(\unfoldexp(e_0', e_0), \ldots, %
              \unfoldexp(e_n, e_n'))$ } \label{line:recurse} }
        } %
      } \label{line:infer-aux-end} }
    \Return{$\inferaux(\initunfold(e_0), \ldots, \initunfold(e_n))$ %
      \label{line:infer-base} }
  }
  % 
  \caption{ \sys: a type-inference procedure that uses procedures
    \inferbnd, \chkinductive, $\initunfold$, and \unfoldexp. }
  \label{alg:infer}
\end{algorithm}
\end{figure}
...
The procedures $\initunfold$ and \unfoldexp are straightforward to
implement.
%
One satisfactory implementation of $\initunfold$, given $e \in \expr$,
iteratively replaces sub-expressions of $e$ of the form $\fix{f}{e'}$
with $e'$.
%
One implementation of \unfoldexp, given $e, e' \in \expr$ such that
$e' \unfolding e$, replaces each occurrence of fixpoint variable $f$
in $e'$ where $\fix{f}{e''}$ is a sub-expression of $e$, with
$\initunfold(e'')$.
%
