...
For indices $I$ and $X \subseteq \lfpvars$, an \emph{relational
  refinement type representation} over $I$ and $X$ is a triple
$(\Phi, P, E)$, where
...
%
\textbf{(2)} $P : X \to I \times \typos$;
%
\textbf{(3)} $E \subseteq X \times X$ is an equivalence relation over
$X$.
%
...
%
% introduce map
For indices $I$, $X \subseteq \lfpvars$ and %
indexed refinement predicates $\Phi : I \to \refpreds{I}$, let
$\Phi' : \idxpos{I} \partto \refpreds{I}$ be such that for each
$i \in I$,
$\Phi'(i, \epsilon) = \Phi(i)[ \setformer{ (i, \epsilon) / i }{ i \in
  I } ]$.
%
For $P : X \to I$, let $P' : X \to I \times \typos$ be such that for
each $i \in I$, $P'(i) = P(i) \times \epsilon$.
%
Then for each $\mathcal{M} \subseteq X$, the \emph{atomic} relational
type defined by $\Phi$, $P$, and $\mathcal{M}$ is
$(\Phi', P', \mathcal{M})$.
...
$\rename{\varphi}{i}{j} = \varphi[ \setformer{(j, p) / (i, p)}{ p \in
  \typos} ]$.
% define index renaming over sets of indexed positions:
For $J \subseteq \idxpos{I}$, the renaming of $J$ from $i$ to $j$ is
$\rename{J}{i}{j} = \setformer{(k, p)}{ k \not= j \in J, p \in \typos,
  (k, p) \in J} \union \setformer{ (j, p) }{ p \in \typos, (i, p) \in
  J}$.
% define renaming over sets of predicates,
For $\Phi : \idxrefpreds{I}$, the renaming of $\Phi$ from $i$ to $j$
is denoted $\rename{\Phi}{i}{j} = %
\setformer{ \rename{J} \mapsto \rename{\Phi(J)}{i}{j} }{ %
  J \subseteq \idxpos{I}, J \in \domain(\Phi) }$.
...
$\mathcal{M} \in \equiv_{\lfpvars}$, the abstraction of
$(\Phi, \mathcal{M})$ over $x$ at $i$ is
$\to_x^i (\Phi, \mathcal{M}) = (\Phi', \mathcal{M})$, where
