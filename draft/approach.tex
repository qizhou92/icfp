...
\begin{figure}
  ...
  \begin{gather*}
    ...
    \inference[ FN ]{ 
      ...
      \upd{ T }{ p }{ \sigma \to \sigma' } \reltyof \tau \to_x^p \tau' } 
    \inference[ PROD ]{
...
follows.
%
Rule~ATOMS establishes \BH{complete}

% functions:
Rule~FN establishes that if 

\BH{walk through rules}
...
\begin{figure}
  ...
  \begin{gather*}
    ...
    % join is symmetric on first two components:
    \inference[ SYM ]{ %
      \tau' \join{ \varphi } \tau \subtype_V \tau'' }{ %
      \tau \join{ \varphi } \tau' \subtype_V \tau'' } \\
    % join fixpoints:
...
\BH{rework this so that join is parameterized on indexed variance, to
  support lfp}
