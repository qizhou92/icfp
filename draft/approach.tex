...
\begin{defn}
  ...
  are \emph{relatable in $\tau$} if one of the following conditions
  holds:
  \begin{itemize}
  \item % identity:
...
operators over individual algebraic types, generalizations of type
operators used in refinement type systems, and operators introduced to
operate over features unique to relational types.

% define relational sum:
\paragraph{Sum of relational types} The relational sum of types $\tau$
and $\tau'$ from indices $i$ and $j$ to index $k$ is a relational
refinement type in which the type at $k$ is the sum of types at $i$ in
$\tau$ and at $j$ in $\tau'$.
%
I.e., for space of indices $I$, indices $i, j, k \in I$,
$\Phi, \Phi' \subseteq \relpreds{ \idxpos{I} }$, and
$M, M' \subseteq \pset(\idxpos{I})$, the sum of $(\Phi, M)$ and
$(\Phi', M')$ from $i$ and $j$ to $k$, denoted $\tau +_{i, j}^k \tau'$
is $(\Phi'', M'')$, where
%
\begin{itemize}
\item 
  \BH{define predicates}
\item
  \BH{define complete induction set}
\end{itemize}
%
It is straightforward to prove that if $(\Phi, M)$ and $(\Phi', M')$
are relational refinement types, then
$(\Phi, M) +_{i, j}^k (\Phi', M')$ is a relational refinement type.

% define relational product:
\paragraph{Product of a relational type}
...
%
I.e., the product of $\tau$ from $i$ and $j$ to $k$, denoted
$\times_{i, j}^k \tau$, is
%
\begin{itemize}
\item % define predicates
  \BH{define predicates}
\item
  \BH{define complete induction set}
\end{itemize}
%
It is straightforward to prove that if $(\Phi, M)$ is a relational
refinement types, then $\times_{i, j}^k (\Phi, M)$ is a relational
refinement type.

\paragraph{Unfolding of a relational refinement type}
