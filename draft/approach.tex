...
\begin{algorithm}[t]
  ...
  { \myproc{$\verifyaux(D_0, D_1, R)$ \label{line:aux-begin} }{ %
      ...
      \Switch{$\chkinductive(D_0, D_1, R)$ \label{line:chkind} }{ %
        ...
        \Case{$D_0' \in \derdagsof{ \cc{P}_0 }, %
          D_1' \in \derdagsof{ \cc{P}_1 }$ %
          \label{line:case-cex}}{ %
  ...
  %
  \caption{% verify interface:
    \verify: given $P_0, P_1 \in \stlc$, decides if $P_0 \equiv P_1$.
    % procedures used: 
    \verify uses procedures \mergeinvs (\autoref{sec:der-invs}), %
...
\subsubsection{Representing relational invariants of derivations}
\label{sec:der-invs}
\BH{give representation of sets of derivations}

\subsubsection{Checking inductiveness of derivation invariants}
\label{sec:chk-ind}

\subsubsection{Verifying derivation equivalence}
