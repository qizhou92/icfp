...
%
The space of all instantiations of $\evalrules$ in $P$ is denoted
$\instrules{P}$.
...
\end{defn}
%
For each $e \in \subexps{P}$, all $\mathcal{D} \in \dertrees{P}{e}$,
and each $r \in \instrules{P}$ such that $(\mathcal{D}, r) \in
\dertrees{P}{e}$, $\mathcal{D}$ are the \emph{immediate
  sub-derivations} of $(\mathcal{D}, r)$.
%
\BH{keep?}
...
%
The space of derivation contexts of $P$ is denoted $\derctxs{P} =
(\instrules{P} \times \nats)^{*}$.
...
\nu \sats \psi$.
%
In such a case, $\psi$ is denoted $\symstep{ P }{ r }$.
